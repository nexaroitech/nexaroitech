...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Mobile dropdown toggle (Services / Internship) ──
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async function(e) {
      ...
      const waUrl = `https://wa.me/916207631281?text=${encodeURIComponent(waText)}`;
      window.open(waUrl, '_blank');

      // Also send via mailto fallback
      const mailtoLink = `mailto:hr@nexaroitech.com?subject=${encodeURIComponent('[Nexaro] ' + data.subject)}&body=${encodeURIComponent(`Name: ${data.name}\nEmail: ${data.email}\nPhone: ${data.phone}\n\n${data.message}`)}`;
  ...
  if (internForm) {
    internForm.addEventListener('submit', function(e) {
      ...
      const mailtoLink = `mailto:hr@nexaroitech.com?subject=${encodeURIComponent('[Internship Application] ' + data.domain + ' – ' + data.fullname)}&body=${encodeURIComponent(`Name: ${data.fullname}\nEmail: ${data.email}\nPhone: ${data.phone}\nDomain: ${data.domain}\nCollege: ${data.college}\nLinkedIn: ${data.linkedin}\n\n${data.message}`)}`;
      setTimeout(() => window.open(mailtoLink, '_blank'), 1500);
